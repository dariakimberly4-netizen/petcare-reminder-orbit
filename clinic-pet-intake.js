...
document.addEventListener('click',e=>{const add=e.target.closest('[data-register-clinic-pet]');if(add){e.preventDefault();e.stopImmediatePropagation();registerForm();return}const b=e.target.closest('[data-open-clinic-pet]');if(b){e.preventDefault();profile(b.dataset.openClinicPet)}},true);
